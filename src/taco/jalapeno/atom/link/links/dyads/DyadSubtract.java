...
public class DyadSubtract extends DyadicLink {
		...
		if(a instanceof VarList){
			...
			if(b instanceof VarList){
				...
				if(a instanceof VarChar || b instanceof VarChar){
					return new VarChar((char) a.toNumber().data.subtract(b.toNumber().data).intValue());
				}else{
					return new VarNumber(a.toNumber().data.subtract(b.toNumber().data));
				}
			}
		}
	}
