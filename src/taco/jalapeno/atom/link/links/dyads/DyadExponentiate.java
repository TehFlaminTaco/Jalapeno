...
public class DyadExponentiate extends DyadicLink {
		...
		if(a instanceof VarList){
			...
			if(b instanceof VarList){
				...
				if(a instanceof VarChar || b instanceof VarChar){
					return new VarChar((char) a.toNumber().data.pow(b.toNumber().data.intValue()).intValue());
				}else{
					return new VarNumber(a.toNumber().data.pow(b.toNumber().data.intValue()));
				}
			}
		}
	}
